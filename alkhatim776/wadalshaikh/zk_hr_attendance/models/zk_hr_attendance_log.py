...
class ZKMachineAttendanceLog(models.Model):
    ...
    machine_employee_id = fields.Integer("ZK Machine Employee ID", required=1)
    ...
    def _compute_attendance_log_name(self):
        for rec in self:
            employee = rec.env['hr.employee'].search([('biometric_device_id', '=', rec.machine_employee_id)])
            if employee:
                    ...
                    if len(all_current_date_attendances) > 0:
                        ...
                        if len(current_date_hr_attendances) > 0:
                            ...
                            self.env['hr.attendance']. \
                                create({"employee_id": employee.id, "check_in": current_date_attendances[0].
                                       attendance_timestamp, "check_out": current_date_attendances[-1].
                                       attendance_timestamp, "company_id": employee.company_id.id})
                ...
                for date_of_attendance in dates_of_attendance:
                    ...
                    if len(all_current_date_attendances) > 0:
                        ...
                        for attendance in all_current_date_attendances:
                            if attendance.operation == 1 and last_date_attendance:
                                last_date_attendance.write(
                                    {"check_out": attendance.attendance_timestamp})
                            elif attendance.operation == 0:
