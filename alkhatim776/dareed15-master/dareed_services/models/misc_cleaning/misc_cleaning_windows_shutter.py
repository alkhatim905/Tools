...
class MiscCleaningWindowsShutter(models.Model):
    _name = "misc.cleaning.windows.shutter"

    # Main Data
    name = fields.Char(string="Window shutters (both sides):")
        ...
        for record in self:
            if record.level_of_dirtiness:
                product_id = self.env['product.product'].search([
                    ('name', 'ilike', "Window shutters (both sides)"),
